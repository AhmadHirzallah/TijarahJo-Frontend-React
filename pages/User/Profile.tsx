...
const Profile: React.FC = () => {
  ...
  const [newPasswordValidation, setNewPasswordValidation] = useState<ReturnType<
  ...
  const handleChangePassword = async (e: React.FormEvent) => {
    ...
    const validation = validatePassword(passForm.newPassword);
    if (!validation.isValid) {
      setPassError(validation.error || "Password does not meet requirements.");
      return;
    }
  ...
  const handlePhoneAction = async (action: 'add' | 'delete' | 'primary', payload?: any,phoneNumber?:any) => {
    ...
    try {
      ...
      if (action === 'primary') await api.put(`/users/${profile.userID}/phones/${payload}`,{
                  ...
                  <div key={p.phoneID} className={`p-8 rounded-[3rem] flex items-center justify-between transition-all border-2 ${p.isPrimary ? 'bg-indigo-50 border-indigo-200' : 'bg-slate-50/50 border-transparent hover:border-slate-100'}`}>
                    ...
                    <div className="flex gap-3">
                       ...
                       {!p.isPrimary && (
                         ...
                         <button onClick={() => handlePhoneAction('primary', p.phoneID,p.phoneNumber)} className="p-4 text-slate-400 hover:text-amber-500 hover:bg-white rounded-2xl transition-all shadow-sm" title="Set Primary"><Star size={20}/></button>
                        ...
                        <form onSubmit={handleChangePassword} className="space-y-4 animate-in slide-in-from-top-4">
                           ...
                           <div className="space-y-1">
                              ...
                              <input
                                ...
                                className={`w-full px-5 py-4 bg-white/5 border rounded-xl outline-none text-sm transition-all ${
                              ...
                              {passForm.newPassword && newPasswordValidation && (
                                <div className="mt-2 space-y-2">
                                  {newPasswordValidation.strength && (
                                    <div className="flex items-center gap-2">
                                      <div className="flex-1 h-1.5 bg-slate-200 rounded-full overflow-hidden">
                                        <div
                                          className={`h-full ${getPasswordStrengthBg(
                                            newPasswordValidation.strength
                                      ...
                                      <span
                                        className={`text-[10px] font-black uppercase tracking-widest ${getPasswordStrengthColor(
                                          newPasswordValidation.strength
                                        )}`}
                                      >
                                        {newPasswordValidation.strength}
                                  ...
                                  {newPasswordValidation.requirements && (
                                    <div className="grid grid-cols-2 gap-1 text-[9px]">
                                      <div
                                        className={`flex items-center gap-1.5 ${
                                          newPasswordValidation.requirements.minLength
                                            ? "text-emerald-600"
                                            : "text-slate-400"
                                        }`}
                                      >
                                      ...
                                      <div
                                        className={`flex items-center gap-1.5 ${
                                          newPasswordValidation.requirements.hasUpperCase
                                            ? "text-emerald-600"
                                            : "text-slate-400"
                                        }`}
                                      >
                                      ...
                                      <div
                                        className={`flex items-center gap-1.5 ${
                                          newPasswordValidation.requirements.hasLowerCase
                                            ? "text-emerald-600"
                                            : "text-slate-400"
                                        }`}
                                      >
                                      ...
                                      <div
                                        className={`flex items-center gap-1.5 ${
                                          newPasswordValidation.requirements.hasNumber
                                            ? "text-emerald-600"
                                            : "text-slate-400"
                                        }`}
                                      >
                                      ...
                                      <div
                                        className={`flex items-center gap-1.5 col-span-2 ${
                                          newPasswordValidation.requirements.hasSpecialChar
                                            ? "text-emerald-600"
                                            : "text-slate-400"
                                        }`}
                                      >
