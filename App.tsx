...
export default function App() {
  ...
  useEffect(() => {
    ...
    setProductDetailsOrigin,
    setProductDetailsOriginCategory,
  ]);
      ...
      <EditProfilePage
        ...
        onSave={async (updatedProfile) => {
          try {
            ...
            if (Number.isNaN(userIdInt) || userIdInt < 1) {
              toast.error("Invalid user ID. Please log in again.");
              return;
            }

            // The backend accepts only editable profile fields. Password hashes,
            // roles, account status, and deletion flags never cross this boundary.
            const userUpdateData = {
              ...
              Area: updatedProfile.area.trim() || null,
              Bio: updatedProfile.bio.trim() || null,
              Avatar: updatedProfile.avatar?.trim() || null,
            };

            // Call backend API to update user
            try {
              const updatedUserData = await api.users.updateUser(userId, userUpdateData);
              const savedUser = updatedUserData as any;

              // Update local state with saved data
              setUserProfile({
                id: updatedProfile.id,
                name: `${savedUser.FirstName ?? updatedProfile.firstName} ${
                  savedUser.LastName ?? updatedProfile.lastName
                }`.trim(),
                firstName: savedUser.FirstName ?? updatedProfile.firstName,
                lastName: savedUser.LastName ?? updatedProfile.lastName,
                username: savedUser.Username ?? updatedProfile.username,
                email: savedUser.Email ?? updatedProfile.email,
                phone: savedUser.Phone ?? updatedProfile.phone,
                city: savedUser.City ?? updatedProfile.city,
                area: savedUser.Area ?? updatedProfile.area,
                location: savedUser.City ?? updatedProfile.city,
                bio: savedUser.Bio ?? updatedProfile.bio,
                avatar: savedUser.Avatar ?? updatedProfile.avatar ?? "",
