...
// Images are stored as data URLs. Keeping the binary file at or below 1 MiB
// leaves enough room for base64 expansion under the API's 2,000,000-character
// PostImageURL limit.
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files) {
      const selectedFiles = Array.from(files).slice(
        0,
        Math.max(0, 5 - selectedImages.length)
      );
      ...
      if (invalidFile) {
        toast.error(
          language === "ar"
            ? "يجب أن تكون كل صورة بصيغة صورة وحجمها 1 ميجابايت أو أقل"
            : "Each image must be an image file no larger than 1 MB"
        );
      }
